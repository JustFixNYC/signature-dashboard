...
import { Borough } from "../../types/APIDataTypes";
import "./style.scss";

const GeoDownshift = Downshift;
...
export interface AddressSearchProps {
  onFormSubmit: (searchAddress: SearchAddress, error: unknown) => void;
...
function toSearchAddresses(results: GeoSearchResults): SearchAddress[] {
  return results.features.map((feature) => {
    const formattedBoroName =
      feature.properties.borough.toUpperCase() as Borough;
...
> {
  ...
  constructor(props: AddressSearchProps) {
    ...
    this.requester = new GeoSearchRequester({
      ...
      onResults: (results) => {
        console.log("bbls", this.props.bbls);
        console.log(results);
        results.features = results.features.filter((building) => {
          return this.props.bbls.includes(building.properties.addendum.pad.bbl);
        });
        this.setState({
          isLoading: false,
    ...
    if (results.length > 0) {
      ...
      if (event.keyCode === KEY_TAB && !event.shiftKey) {
        ds.setHighlightedIndex(
          ds.highlightedIndex === results.length - 1 ||
            ds.highlightedIndex === null
            ? 0
              ...
              <div className="form-group col-xs-12">
                <div className="geosuggest">
                  ...
                  <div className="geosuggest__suggests-wrapper">
                    <ul
                      className={suggestsClasses.join(" ")}
